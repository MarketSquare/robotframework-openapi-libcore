...
class OpenApiLibCore:  # pylint: disable=too-many-instance-attributes
    ...
    def authorized_request(  # pylint: disable=too-many-arguments
        ...
        files = [(api_param, (name, open(file_path,"rb"), mimetypes.guess_type(file_path)[0])) for api_param, fd in file_data.items() for name, file_path in fd.items()] if file_data else []
